...
- Introduction of `OfficeStamperEvaluationContextFactory` that produces a per‑hook evaluation context powering
...
- New union accessors and resolvers: `UnionPropertyAccessor`, `UnionIndexAccessor`, and `UnionMethodResolver`
  (collectively “union resolvers”) to transparently resolve properties, indexed values, and method calls across stacked
  contexts.
...
### Context selection: int reference → String key (BREAKING)

In v2, some internals and drafts used the notion of a numeric “context reference” to select a context for a hook
(comment or tag) within a nested processing flow. In v3 this has been formalized and simplified into a
String-based context key available directly on hooks.

What changed:

- Hooks expose a `String` key instead of an integer reference:
    - `Comment#getContextKey()` / `setContextKey(String)`
    - `Tag#getContextKey()` / `setContextKey(String)`
- Context selection uses that key through the context tree:
    - `ContextTree#find(String key)` returns the appropriate `ContextBranch` to evaluate the hook.

Why:

- Improves readability and template intent (named scopes are self-descriptive).
- Decouples selection from positional or implicit indices.
- Enables richer naming schemes and future-proofing of nested scopes.

Migration guidance:

- If you interacted with hooks programmatically and previously relied on an integer reference, migrate to string keys.
  Example (before/after pseudo-code):

  ```java
  // Before (conceptual):
  // var branch = contextTree.find(ref /* int */);

  // After (v3):
  var branch = contextTree.find(key /* String */);
  ```

- Custom processors that created or forwarded sub-contexts should assign meaningful keys when pushing new contexts, and
  read keys from `Comment`/`Tag` when resolving:

  ```java
  var key = comment.getContextKey();
  var branch = contextTree.find(key);
  // evaluate using branch
  ```

Notes:

- Out-of-the-box processors already emit and consume string keys; templates do not require changes unless you
  previously encoded assumptions about numeric references.
-
If you never touched context selection explicitly, you can ignore this change; the engine derives sensible defaults.
...
- Removed `ExpressionResolver` in favor of using `SpelExpressionParser` directly inside `Engine` (BREAKING). If you
  ...
  No behavior change is expected beyond the constructor signature. Evaluation still uses SpEL and the per‑hook
  `EvaluationContext` produced by `OfficeStamperEvaluationContextFactory`.
- **`Comment` interface update**: Removed `getDocument()` as it is no longer needed.
- **`CommentUtil` update**: `createSubWordDocument` now requires `sourceDocument` as a parameter.
...
- Custom or alternative parsers can be used as long as they implement Spring’s `ExpressionParser` interface.
...
- Interface rename and responsibility shift:
    - `EvaluationContextConfigurer#configureEvaluationContext(StandardEvaluationContext ctx)` → replaced by
...
- Each evaluation context is now created fresh for a given root object/branch, avoiding accidental cross‑contamination
  between processing steps.
- Safer defaults: the default factory restricts dangerous features (constructor/method resolution, type lookup, bean
  resolution) while still allowing map and data‑binding accessors.
