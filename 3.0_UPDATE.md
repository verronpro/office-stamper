Office‑stamper 3.0 is approaching release. This version modernizes the core processing algorithm and configuration API.
It delivers more consistent behavior and clearer extension points, but introduces some breaking changes you should
...
- Processors receive a `ProcessorContext` and should push/pop data onto the branch when creating nested scopes.
...
- Review custom `CommentProcessor` implementations: remove reliance on `ParagraphPlaceholder` injection; use
  `ProcessorContext` and `PlaceholderReplacer` instead.
