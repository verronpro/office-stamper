...
private void processTextualPart(DocxPart part, Object contextRoot) {
    ...
    while (iterator.hasNext()) {
        ...
        if (hook.isPresent()) {
            var h = hook.get();
            var evaluationCtxFactory = computeEvaluationContext(); // OfficeStamperEvaluationContextFactory
            if (h.run(engineFactory, contextTree, evaluationCtxFactory)) iterator.reset();
        }
    }
}
----
...
`ExpressionResolver` uses Spring Expression Language (SpEL).
...
* `SpelParserConfiguration` is still passed into `SpelExpressionParser` to tweak language features.
'''
