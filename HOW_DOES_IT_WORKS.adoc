...
- Resolves the applicable context branch via `ContextTree` using the comment’s context key.
...
Note on context keys (v3):

- Both `Comment` and `Tag` expose a `String` context key (`getContextKey()` / `setContextKey(String)`).
- The key is used to locate the evaluation branch via `ContextTree#find(String)`:

[source,java]
----
var key = hook instanceof CommentHook ch ? ch.comment().getContextKey() : ch.tag().getContextKey();
var branch = contextTree.find(key);
var evalCtx = officeStamperEvaluationContextFactory.create(processorContext, branch);
----

- Keys make nested scope selection explicit and readable; older drafts that mentioned integer “context references” are superseded by string keys.
