...
The latest release is v3.0.0. See the link:{docs}/release-notes.html[Release Notes] for details.
...
* Hierarchical scopes: `ContextTree`/`ContextBranch` resolve placeholders along the current branch; unresolved values bubble up to parent scopes.
...
* New `Insert` API for multi‑run insertions; `PlaceholderReplacer` removed in favor of context‑stack management.
