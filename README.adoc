...
The latest release is v3.0. See the link:{docs}/release-notes.html[Release Notes] for details.
...
* Postprocessors added: `cleanTags` can remove all smart tags used by the engine after processing.

Breaking API highlights:

* `AbstractCommentProcessor` → `CommentProcessor`; introduce `CommentProcessorFactory`.
* `ContextDependent` renamed to `Hook`.
