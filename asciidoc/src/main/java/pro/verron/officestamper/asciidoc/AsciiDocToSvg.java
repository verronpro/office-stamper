...
/// Renderer converting an [AsciiDocModel] into an SVG document simulating a Word editor interface.
public final class AsciiDocToSvg
    ...
    public String apply(AsciiDocModel model) {
        ...
        for (int i = 0;
            ...
            currentY = renderBlock(svgContent, block, PAGE_LEFT + PAGE_PADDING, currentY, isCommented);
        ...
        // Background
        fullSvg.append("<rect x=\"0\" y=\"0\" width=\"100%\" height=\"100%\" fill=\"#e6e6e6\"/>\n");

        // Word Top Banner
        renderWordBanner(fullSvg,
                model.getAttributes()
        ...
        renderComments(fullSvg, blockToComments, blockPositions);
    ...
    private void renderWordBanner(StringBuilder svg, String title) {
        ...
        svg.append(
                "<text x=\"20\" y=\"60\" font-family=\"Segoe UI, Arial\" font-size=\"11\" fill=\"#333\">File  Home  "
                + "Insert  Layout  References  Review  View</text>\n");
        svg.append("<rect x=\"20\" y=\"70\" width=\"40\" height=\"20\" fill=\"#2b579a\" rx=\"2\"/>\n");
        svg.append(
                "<text x=\"25\" y=\"84\" font-family=\"Segoe UI, Arial\" font-size=\"10\" "
                + "fill=\"white\">Paste</text>\n");
    }

    private int renderBlock(StringBuilder body, Block block, int x, int y, boolean highlight) {
        int contentStartIdx = body.length();
        int nextY = y;
        switch (block) {
            case Heading(_, int level, List<Inline> inlines) -> {
                int fontSize = Math.max(18, 34 - (level * 3));
                nextY = appendTextLine(body, renderInlines(inlines), x, y + fontSize, fontSize, 700);
                nextY += 10;
            }
            case Paragraph(_, List<Inline> inlines) -> {
                nextY = appendTextLine(body, renderInlines(inlines), x, y + BODY_FONT_SIZE, BODY_FONT_SIZE, 400);
                nextY += 8;
            }
            ...
            case CodeBlock(String language, String content) -> {
                ...
                nextY = appendTextLine(body, "[" + language + "]", x + 10, nextY + BODY_FONT_SIZE, 12, 600);
                for (String line : content.lines()
                                          .toList()) {
                    nextY = appendTextLine(body, line, x + 10, nextY + 12, 13, 400);
                }
                nextY += 8;
            }
        ...
        if (highlight) {
            String highlightRect = String.format(
                    "<rect x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" fill=\"#fff2cc\"/>\n",
                    x - 5,
                    y - 2,
    ...
    private int appendTextLine(StringBuilder body, String line, int x, int y, int fontSize, int weight) {
        body.append("<text x=\"")
            .append(x)
            .append("\" y=\"")
            .append(y)
            .append("\" font-family=\"Calibri, Arial, sans-serif\" font-size=\"")
            .append(fontSize)
            .append("\" font-weight=\"")
            .append(weight)
            .append("\" fill=\"#111\">")
    ...
    private List<CommentInfo> extractComments(AsciiDocModel model) {
        ...
        for (Block block : model.getBlocks()) {
            if (block instanceof MacroBlock(String name, String id, List<String> list) && name
                                                  .equals("comment")) {
                String start = "";
    ...
    private void renderComments(
        ...
        for (BlockPosition pos : blockPositions) {
            ...
            if (comments != null) {
                for (CommentInfo c : comments) {
                    svg.append(String.format(
                            "<rect x=\"%d\" y=\"%d\" width=\"%d\" height=\"60\" fill=\"#f9f9f9\" stroke=\"#ffc000\" "
                            + "stroke-width=\"1\" rx=\"4\"/>\n",
                            COMMENTS_LEFT,
                            commentY,
                            COMMENT_WIDTH));
                    svg.append(String.format(
                            "<text x=\"%d\" y=\"%d\" font-family=\"Segoe UI, Arial\" font-size=\"11\" "
                    ...
                    svg.append(String.format(
                            "<line x1=\"%d\" y1=\"%d\" x2=\"%d\" y2=\"%d\" stroke=\"#ffc000\" stroke-width=\"1\" "
                            + "stroke-dasharray=\"4\"/>\n",
                            startX,
                            startY,
                            endX,
