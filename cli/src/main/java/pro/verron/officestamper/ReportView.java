...
import pro.verron.officestamper.TraceabilityReport.Resolution;
...
@Command(
        name = "report-view", description = "Generate an HTML viewer for a traceability report"
)
public class ReportView implements Runnable {

    @Option(
            names = {"-i", "--input"}, required = true, description = "JSON traceability report"
    )
    private Path input;

    @Option(
            names = {"-o", "--output"}, defaultValue = "traceability.html", description = "Output HTML file"
    )
    private Path output;
    ...
    public void run() {
        try {
            ...
            if (resolutionsNode == null) throw new OfficeStamperException("Could not find resolutions in report");

            List<Resolution> resolutions = mapper.convertValue(resolutionsNode, new TypeReference<>() {
            });
    ...
    private String generateHtml(List<Resolution> resolutions) {
        ...
        for (var res : resolutions) {
            sb.append("<tr>");
            sb.append("<td><code>").append(res.expression()).append("</code></td>");
            sb.append("<td>").append(res.value()).append("</td>");
            sb.append("<td><ul>");
            for (var ctx : res.contextStack()) {
                sb.append("<li>").append(ctx).append("</li>");
            }
            sb.append("</ul></td>");
            sb.append("</tr>");
        }
