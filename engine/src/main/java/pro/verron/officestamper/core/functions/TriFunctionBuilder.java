...
public class TriFunctionBuilder<T, U, V>
    ...
    /// Constructs a new instance of TriFunctionBuilder. This constructor initializes the TriFunctionBuilder with the
    /// given configuration, name, and input types. It prepares the builder to define and register a custom
    /// TriFunction.
    ///
    ...
    /// and `V`, and produces a result. The provided implementation is encapsulated as a `CustomFunction` and added to
    /// the underlying configuration for later use.
    ///
    /// @param implementation the `TriFunction` implementation to register.
    ///
    ///                 This function takes three input arguments of types `T`, `U`, and `V` and produces a result.
    @Override
