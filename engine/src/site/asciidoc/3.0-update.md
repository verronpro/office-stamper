...
[Back to Index](index.html) | [Release Notes](release-notes.html)

This document tracks major changes and migration steps for recent versions of Office-stamper.

## 🚀 Migration from 3.2 to 3.3

Office-stamper 3.3 introduces built-in SVG support, new utility methods for smart tag validation, and a leaner codebase
following internal refactoring.

### 🖼️ SVG Support (NEW)

The `Image` class and engine now support SVG images. The library automatically detects SVG content and uses a new
`WmlFactory.newSVGDrawing` method for rendering. This allows you to use vector graphics in your templates.

### 🛠️ Smart Tag Attribute Validation (NEW)

A new utility method `WmlUtils.hasTagAttribute` has been added to the `utils` module. This helps in validating the
presence and values of attributes within DOCX smart tags, providing more robust control over document structure
manipulation.

### 🧹 Internal API Cleanup (REFACTOR)

As part of a continuous effort to streamline the library, the following classes have been removed from the `utils`
module:

- `ByteUtils`: Replaced by standard Java or other internal alternatives.
- `DocxRenderer`: Removed as its functionality was refactored into other components.

### 📦 Dependency Updates

Key dependencies have been updated:

- Spring Expression and Context: 7.0.6
- Jackson: 2.21.2
- PIT mutation testing: 1.23.0
- Mockito JUnit Jupiter: 5.23.0
...
Office-stamper 3.0 modernized the core processing algorithm and configuration API. It requires Java 23+ (Maven release
set to 25).

## 🔄 What changed (at a glance)

### Processing model overhaul
...
New hook concept: each placeholder/comment is treated as a hook — the iterator walks hooks one by one. Document order
processing ensures a more predictable and intuitive flow.
...
### Hierarchical scopes and context
...
- `ContextTree` resolve placeholders along the current branch; unresolved values bubble up to parent scopes.
- Processors receive a `ProcessorContext` and should iterate over the hooks context they want to update with nested
  scopes.
- Cleaner separation of concerns: processors focus on document changes; the engine handles expression evaluation.
- Preprocessor-driven placeholders: include the placeholder preprocessor in your `OfficeStamperConfiguration` (already
  present in standard presets) to enable resolution.
- Comment processor lifecycle simplified: processors are created per comment and executed once.
- Postprocessors added: `Postprocessors.removeTags(String)` can remove all smart tags used by the engine after
  processing.
...
- New docs: `how-to-custom.html` and `HOW_DOES_IT_WORKS.adoc`
