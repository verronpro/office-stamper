...
* **Expression API**: Replaced `ExpressionParser` with a new `Expression` abstraction to improve internal handling and support different expression languages.

==== New Features

* **Table and Row Manipulation**: Introduced new `Table` and `Table.Row` interfaces in the public API for easier structural modification of documents.
* **Comment Removal**: Added `CommentRemover` post-processor for automatic document cleanup.
* **Configuration Presets**: Enhanced `OfficeStamperConfigurations` with `minimal()`, `standard()`, and `full()` presets.

==== AsciiDoc Enhancements

* **Conversion Improvements**: Major updates to the AsciiDoc module with better support for nested tables, hyperlinks, headers, and footers.
