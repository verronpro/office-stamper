...
* **Image API Refactor**: The `Image` class constructors now use `InputStream` for improved flexibility and memory efficiency.
Internal byte caching and deduplication ensure that identical images are only stored once in the generated document.
...
* **Documentation Domain**: The documentation website has moved to a new domain: link:https://office-stamper.verron.pro/[office-stamper.verron.pro]
...
The `Image` class has been updated to automatically detect SVG content and handle its insertion using a new `WmlFactory.newSVGDrawing` method.
...
* **Expression API Refactor**: Replaced `ExpressionParser` with a new `Expression` abstraction.
This improves the internal handling of expressions and allows for better support of different expression languages.
* **Table and Row Manipulation**: Introduced new `Table` and `Table.Row` interfaces to the public API.
Custom processors can now easily remove tables or rows, copy rows, and access parent table/row structures directly from a paragraph.
...
* **Java Version**: Java 23+ is required; Maven release set to 25.
* **Package Moves**:
...
* New hierarchical scopes with `ContextTree`.
...
** It may break the formatting of the following paragraphs if there was an empty run with formatting instruction inside the comment selection, but I have not been able to reproduce this theoretical issue with both Word, and WPS writer.
Do not hesitate to inform me if you encounter the issue.
...
* Massive rework of the documentation, now most of it is on the https://office-stamper.verron.pro/[site]
...
Try using `OfficeStamperConfiguration.addCustomFunction` methods.
...
and allows users to handle errors programmatically as was expressed in the old https://github.com/verronpro/office-stamper/issues/65[Issue #65]
...
* Added a regression test for https://github.com/verronpro/office-stamper/issues/64[Issue #64]
