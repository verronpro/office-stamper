...
The `Image` class and engine now automatically detect SVG content and handle its insertion using a new `WmlFactory.newSVGDrawing` method.

==== New Features

* **Smart Tag Attribute Validation**: New utility method `WmlUtils.hasTagAttribute` to check for attributes within DOCX smart tags.

==== Cleanup & Refinement

* **Internal refactoring**: Simplified the `utils` module by removing legacy classes like `ByteUtils` and `DocxRenderer`.

==== Dependencies

* **Spring**: Updated Spring Expression and Context to version 7.0.6.
* **Jackson**: Updated Jackson monorepo to version 2.21.2.
* **Tooling**: Updated PIT mutation testing to 1.23.0.
...
* **SVG Images**: You can now pass SVG content to the `Image` class; the engine will handle the conversion and insertion into the DOCX document automatically.
* **Utility Changes**: If you were using `ByteUtils` or `DocxRenderer` from the `utils` module, you will need to find alternative implementations as these have been removed.

=== From v3.1.0 to v3.2.0

* **Table Manipulation**: Use the new `Table` and `Table.Row` interfaces for custom table-related logic.
* **Configuration**: Use the new presets in `OfficeStamperConfigurations` for a quicker setup.
* **Expression Parser**: If you were using a custom `ExpressionParser`, note that the configuration now uses `SpelParserConfiguration`.
...
* **Internal Classes**: If you were using internal classes like `SectionUtil`, they have been removed.
You will need to implement this logic yourself if needed.
