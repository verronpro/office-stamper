...
/// Test class for the ExcelContext functionality. This class contains unit
/// tests
/// to verify the behavior of the ExcelContext, ensuring proper handling of
/// sheets,
...
///
/// These tests use a predefined sample Excel file (excel-base.xlsx) for
/// verifying
...
@DisplayName("ExcelContext Tests") public class ExcelContextTest {
    ...
    void testInnerJoin() {
        ...
        var right = List.of(Map.of("ID", "1", "Age", "25"),
                Map.of("ID", "2", "Age", "30"));
    ...
    void testInnerJoin_OneToMany() {
        var left = List.of(Map.of("ID", "1", "Name", "Alice"));
        var right = List.of(Map.of("ID", "1", "Task", "T1"),
                Map.of("ID", "1", "Task", "T2"));
    ...
    @Test
    @DisplayName(
            "Should expose sheets by index and A1 cells, and default rows by "
            + "headers (existing sample)")
