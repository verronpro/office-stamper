...
import pro.verron.officestamper.utils.wml.DocxDocument.Comment;
...
public class CommentUtil {
    ...
    /// @throws UtilsException if an error occurs while retrieving the content
    public static Comments extractContent(CommentsPart commentsPart) {
        try {
            return commentsPart.getContents();
        } catch (Docx4JException e) {
            throw new UtilsException("Error while searching comment.", e);
        }
    }
    ...
    public static void deleteComment(Comment comment) {
        ...
        var endParent = (ContentAccessor) end.getParent();
        endParent.getContent().remove(end);

        var start = comment.getCommentRangeStart();
        var startParent = (ContentAccessor) start.getParent();
        startParent.getContent().remove(start);

        if (startParent instanceof CTSmartTagRun tag) if (tag.getContent().isEmpty()) {
            var tagParent = (ContentAccessor) tag.getParent();
            tagParent.getContent().remove(tag);
        }

        var reference = comment.getCommentReference();
        if (reference != null) {
            var referenceParent = (ContentAccessor) reference.getParent();
    ...
    public static Comment comment(Part docxPart, CommentRangeStart crs, DocxDocument document, Parent parent) {
        ...
        var comment = getCommentsPart(document.getPackage().getParts()).map(CommentUtil::extractContent)
                .map(Comments::getComment)
                .stream()
                .flatMap(Collection::stream)
