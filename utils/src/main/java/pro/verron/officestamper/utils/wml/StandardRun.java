...
/// Represents a run (i.e., a text fragment) in a paragraph. The run is indexed relative to the containing paragraph and
/// also relative to the containing document.
///
...
public record StandardRun(int startIndex, R run) {
    ...
    /// Initializes a list of StandardRun objects based on the given list of objects. Iterates over the provided list of
    /// objects, identifies instances of type R, and constructs StandardRun objects while keeping track of their
    /// lengths.
    ///
    ...
    public static List<StandardRun> wrap(ContentAccessor contentAccessor) {
        var currentLength = 0;
        var runList = new ArrayList<StandardRun>(contentAccessor.getContent()
                                                                .size());
    ...
    ///
    /// @return the index of the first occurrence of the specified substring, or &ndash;1 if the substring is not
    ///         found.
    ...
    /// Determines whether the current run is affected by the specified range of global start and end indices. A run is
    /// considered "touched" if any part of it overlaps with the given range.
    ///
    ...
    /// Extracts a substring of the run's text, starting from the beginning and extending up to the localized index of
    /// the specified global end index.
    ///
    /// @param globalEndIndex the global end index used to determine the cutoff point for the extracted
    ///         substring.
    ///
    /// @return a substring of the run's text, starting at the beginning and ending at the specified localized index.
    public String left(int globalEndIndex) {
        return getText().substring(0, localize(globalEndIndex));
    }

    /**
     * Extracts a substring of the run's text, starting from the localized index of the specified global start index to
     * the end of the run's text.
     *
     * @param globalStartIndex the global index specifying the starting point for the substring in the run's
     *         text.
     *
    ...
    /// Converts a global index to a local index within the context of this run. (meaning the index relative to multiple
    /// aggregated runs)
    ///
